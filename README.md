...
| `PostToolUse` + `PostToolUseFailure` (Write/Edit/NotebookEdit/Bash, async) | Marks the session as active and prompts background documentation of non-trivial work via the `reqall-documenter` agent; throttled. Bash counts only when the command can plausibly write — `git status`, `ls`, `cat`, `rg`, `gh pr view` and similar are ignored; compounds, pipes, redirects and substitutions count. Successful Git bookkeeping — a plain `git add`/`commit`/`push`/`fetch`/`pull` or `gh pr create`/`merge`, alone or chained with `&&` — is also ignored: it mutates the repo but is not work worth a record, so a commit-only follow-up turn does not reopen the persist cadence. Git global options, aliases, and any other shell syntax stay conservative, and a failed call — delivered as `PostToolUseFailure` — always counts: a rejected push may be a finding worth recording |
