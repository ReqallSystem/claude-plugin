 ...
 */
import { appendIntent, consumedOwnIds, isIntentKind, parseProjectId, readIntents, readState, readStdin, retireOwnIds, sessionKey, updateState, } from './common.js';
function walk(value, found, depth = 0) {
    ...
    if (typeof value === 'object') {
        ...
        if (Array.isArray(obj.results) && found.poll === undefined)
            found.poll = obj;
...
if (match) {
    ...
    if (op === 'upsert_project' && !failed) {
        ...
        if (own.length > 0 && found.poll !== undefined)
            retireOwnIds(key, consumedOwnIds(found.poll, own));
    }
