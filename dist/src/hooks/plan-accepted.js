 ...
 */
import { attributionNote, emitContext, projectName, readStdin, touchMarker } from './common.js';
const input = readStdin();
if (input.tool_name === 'ExitPlanMode') {
    const sessionId = input.session_id ?? 'global';
    touchMarker(`activity-${sessionId}`);
    const name = projectName(input);
    emitContext('PostToolUse', `[reqall] A plan was just accepted — that is agreed intent. Before starting the work, ` +
...
//# sourceMappingURL=plan-accepted.js.map